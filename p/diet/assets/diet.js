(function () {
  ...
  var ingredientGroups = [
    ...
    },
    {
      ...
      description: "가능한 고기·생선·두부·채소는 먹되, 분리되는 면·떡·감자는 빼고 먹습니다. 설탕·밀가루·전분이 국물과 양념 전체에 섞였으면 건더기만 건져 먹지 말고 음식 자체를 피합니다.",
  ...
  var ingredientDetails = {
    ...
    "두부김치": { verdict: "1주차부터 조건부", tone: "conditional", note: "두부와 무가당·무밀 김치 중심. 설탕·찹쌀풀을 넣고 함께 볶았다면 두부만 골라 먹지 말고 음식 전체를 피하기" },
    ...
    "샤브샤브": { verdict: "1주차부터", tone: "okay", note: "고기·해산물·채소 중심으로 먹고 면·만두·죽은 빼기. 설탕·밀가루가 육수에 섞였다면 음식 전체를 피하기" },
    ...
    "된장찌개": { verdict: "1주차 점심부터 조건부", tone: "conditional", note: "무가당·무밀 된장에 두부·해산물·버섯·애호박을 넣은 것. 감자는 빼면 되지만 된장이나 육수에 밀가루·물엿이 섞였으면 전체를 피하기" },
    "차돌된장찌개": { verdict: "3주차부터 조건부", tone: "conditional", note: "소고기가 추가되는 3주차부터 차돌은 소량, 두부·채소 중심으로 먹기. 감자는 빼고 된장이나 육수에 밀가루·물엿이 섞였으면 전체를 피하기" },
    "달래된장찌개": { verdict: "1주차 점심부터 조건부", tone: "conditional", note: "무가당·무밀 된장과 달래·두부·해산물·채소로 만든 것. 감자는 빼고 된장이나 육수에 밀가루·물엿이 섞였으면 전체를 피하기" },
    "순두부찌개": { verdict: "1주차 점심부터 조건부", tone: "conditional", note: "순두부·계란·해산물 중심. 면은 빼면 되지만 양념장·육수에 설탕·밀가루·전분이 풀렸다면 찌개 전체를 피하기" },
    "청국장찌개": { verdict: "1주차 점심부터 조건부", tone: "conditional", note: "콩·두부·채소 중심의 무가당·무밀 청국장 기준. 감자는 빼고, 장이나 국물에 밀가루·물엿이 섞였으면 전체를 피하기" },
    ...
    "짜글이": { verdict: "일반 제품은 피하기", tone: "avoid", note: "고추장·설탕·물엿 양념이 고기와 국물 전체에 배는 경우가 많아 감자만 빼도 해결되지 않음. 무가당·무밀 간장과 고춧가루로 직접 만든 경우만 다시 판단" },
    ...
    "짬뽕": { verdict: "일반 제품은 피하기", tone: "avoid", note: "밀면은 빼도 되지만 설탕·전분·밀가루가 국물에 섞인 경우가 많아 해산물만 건져 먹지 않기. 무가당·무밀 국물을 확인한 경우만 다시 판단" },
  ...
  function getIngredientDetail(name, group) {
    ...
    if (title.indexOf("1–3일차") === 0) {
      ...
      return { verdict: "1–3일차 목록 식품", tone: "okay", note: "쉐이크 식사 사이에 생으로 먹거나 삶아 보조하고, 기름·설탕·밀가루를 더한 일반식으로 만들지 않기" };
    }

    if (title.indexOf("1주차부터") === 0) {
      if (nameIn(name, ["닭가슴살", "닭고기", "닭다리살", "닭안심", "돼지등심", "돼지뒷다리살", "돼지안심", "돼지앞다리살", "목살"])) {
        return { verdict: "1주차 점심부터", tone: "okay", note: "튀김옷 없이 굽거나 삶고 설탕·물엿·맛술·밀가루가 든 양념은 사용하지 않기" };
      }
      if (nameIn(name, ["가리비", "갈치", "갑오징어", "고등어", "골뱅이", "광어", "굴", "꼬막", "꽃게", "꽁치", "낙지", "농어", "대구", "도미", "멍게", "명태", "문어", "민어", "바지락", "방어회", "산낙지", "삼치", "새우", "생굴", "생선", "성게알", "소라", "아귀", "연어", "연어알", "연어회", "오징어", "우럭", "전복", "조개", "조기", "주꾸미", "참치회", "코다리", "해삼", "해산물", "홍합", "황태"])) {
        return { verdict: "1주차 점심부터", tone: "okay", note: "원물·무양념 기준. 회·구이·찜을 우선하고 초고추장, 달콤한 소스, 밀가루 튀김옷은 제외" };
      }
      ...
      if (nameIn(name, ["강황", "고추", "고춧가루", "꽈리고추", "대파", "마늘종", "청양고추", "풋고추", "허브", "허브티", "후추"])) {
        return { verdict: "1주차부터", tone: "okay", note: "원물이나 무가당 분말을 조미용으로 사용. 시럽·설탕·밀가루가 섞인 양념 제품은 제외" };
      }
      if (nameIn(name, ["계란", "날계란", "메추리알"])) {
        return { verdict: "1주차 점심부터", tone: "okay", note: "계란 자체는 가능하며 설탕·맛술·치즈·밀가루 없이 삶거나 찌기" };
      }
      return { verdict: "1주차 점심부터", tone: "okay", note: "비전분성 채소 원물 기준. 생으로 먹거나 삶고·굽고·가볍게 볶되 설탕·밀가루·달콤한 소스는 제외" };
    }

    if (title.indexOf("2주차부터") === 0) {
      return { verdict: "2주차부터 소량", tone: "conditional", note: "무가당·무밀가루 원물 기준. 한 번 먹을 양을 먼저 덜고 밥·과일·고구마와 한 끼에 과하게 겹치지 않기" };
    }
    ...
    if (title.indexOf("밀·밀가루 원료") === 0) {
      return { verdict: "진행 중 금지", tone: "ban", note: "밀 유래 원료이므로 양이나 주차와 관계없이 제외. 음식에서 완전히 분리되지 않으면 음식 전체를 피하기" };
    }
